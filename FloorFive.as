package 
{
	...
	public class FloorFive extends MovieClip
	{
		...
		public function FloorFive(player:Player,playerState:State)
		{
			_player = player;
		...
		private function onEnterFrame(event:Event)
		{
			...
			if(_player.body.hitTestObject(portal))
			{
				if(MagicTower.gameMode == "play")
				{
					if(_player.numTickets < 1)
					{
						MagicTower.gameMode = "lackofticket";
					    addChild(_shopDialog);
					    _shopDialog.change("喂！那个传送阵每天要消耗掉我很多晶石！想去无尽之境？到我这儿购买通行券！");
					}
					else
					{
						MagicTower.gameMode = "ready";
						addChild(_shopDialog);
					    _shopDialog.change("此次传送将消耗你一个通行券，无尽之境中非常危险，但收益也很大，一切都靠运气！每次重新进入里面的怪物都会复活，祝你好运！");
					}
				}
				if(MagicTower.gameMode == "lackofticket")
				{
				...
				if(MagicTower.gameMode == "ready")
				{
					if(_shopDialog.isShowOver)
					{
						removeChild(_shopDialog);
						_chooseDialog = new ChooseDialog("无尽之境内恐怖异常，建议进入前先保存，并且保证充足生命值。如果生命值不足，可以随时出来。你确认要前往？");
				...
				if(MagicTower.gameMode == "waitchoose")
				{
					if(_chooseDialog.choose == 1)
					{
						...
						--_player.numTickets;
						refreshTickets();
					    MagicTower.gameMode = "enterspecialfloor";
					}
