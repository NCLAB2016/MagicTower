package 
{
	...
	public class FloorSix extends MovieClip
	{
		...
		private var _bottleShow:BottleShow;

		public function FloorSix(player:Player,playerState:State)
		{
			_player = player;
		...
		private function onEnterFrame(event:Event)
		{
			...
			if(bottle && _player.hitTestObject(bottle))
			{
				if(MagicTower.gameMode == "play")
				{
					_player.hasBottle = true;
					refreshHasBook();
					_bottleShow = new BottleShow;
					addChild(_bottleShow);
					MagicTower.gameMode = "waitbottleshow";
				}
				else if(MagicTower.gameMode == "waitbottleshow")
				{
					if(_bottleShow.isShowOver)
					{
						removeChild(_bottleShow);
						_bottleShow = null;
						MagicTower.gameMode = "play";
						_messageDialog.change("得到星之祝福！按L键或者点击状态栏图标使用！");
						addChild(_messageDialog);
						_chimes.play();
						removeChild(bottle);
						bottle = null;
