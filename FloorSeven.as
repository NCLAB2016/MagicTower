package 
{
	...
	public class FloorSeven extends MovieClip
	{
		...
		private var _musicProblem:MusicProblem;
		private var _gmDialog:GMDialog;
		private var _mpMusic:MP_Music;

		public function FloorSeven(player:Player,playerState:State)
		{
			_player = player;
		...
		private function init()
		{
			...
			_musicProblem = new MusicProblem;
            _mpMusic = new MP_Music;
			addChildAt(_playerState, numChildren - 1);
			_playerState.x = _playerState.y = 0;
			_playerState.floor.text = "第六层";
			...
			if(musicKey && _player.body.hitTestObject(musicKey.body))
			{
				...
				if(MagicTower.gameMode == "waitmusic")
				{
					if(_gmDialog.isShowOver)
					{
						...
						_gmDialog = null;
						addChild(_musicProblem);
					    MagicTower.gameMode = "musicproblem";
					}
				}
				if(MagicTower.gameMode == "musicproblem")
				{
					if(_musicProblem.musicState == MusicProblem.SUCCESS)
					{
						removeChild(_musicProblem);
						_player.hasSolvedMusic = true;
						removeChild(ironDoor);
						removeChild(musicKey);
						ironDoor = null;
						musicKey = null;
					...
					else if(_musicProblem.musicState == MusicProblem.FAILURE)
					{
						removeChild(_musicProblem);
						MagicTower.gameMode = "play";
